...
| **AI & Evidence Layer** | `OPERATIONAL (LOCAL / MOCK)` | Thin `AIProvider` interface (defining `generateStructured`) with runtime killswitch covering injected providers, unconditional production mock denial, fail-closed endpoint validation (HTTPS required in production, explicit `baseURL` for non-OpenAI), `MockAIProvider` ($0.00 spend, structural mechanics check without medical accuracy claims unless scripted), OpenAI-compatible provider with structured JSON outputs and transport resilience classifier. Two-call pipeline (CALL 1: candidate generation; CALL 2: evidence-support verification). Bound provider retries (`maxProviderRetries: 0`, max 2 external calls/attempt), verifier evidence deduplication, preflight input limit (`maxVerifierInputChars: 180_000`). Deterministic citation validator enforcing server-derived page numbers. Automated evidence verification QA gate requiring at least 1 summary, 1 objective, 1 concept, and ≥50% supported claims, reducing unsupported-content risk. Safe Server Action error sanitization across all 19 taxonomy codes. Zero vector/pgvector/embeddings. Developer benchmark harness (`pnpm ai:benchmark:study-pack`) evaluating 5 synthetic fixtures in deterministic Mode A Mock Smoke ($0.00 spend; live opt-in requires `--live` or `AI_BENCHMARK_LIVE=true`). |
| **Document Pipeline** | `OPERATIONAL (LOCAL)` | Canonical chunking engine with strict page boundary invariance (single `document_page_id`, `page_number`, `start_char`, `end_char`), 1800 char target (max 2800, 200 overlap), and code-point offset precision. Dedicated Study Pack worker (`src/workers/study-packs-worker.ts`) with `claim_next_study_pack` (`FOR UPDATE SKIP LOCKED`), claim token fencing, lease expiration write revocation, operational retryability (attempt budget refunded on `AI_DISABLED`/`AI_NOT_CONFIGURED`), non-retryable worker internal errors, concurrent enqueue convergence, and archive race closure (cancelling runs and packs, revoking writes with SQLSTATE 55000). 284 Vitest tests pass (228 unit, 56 integration across 19 test files). $0.00 cost. |
| **Study Pack UI** | `OPERATIONAL (LOCAL)` | Scientific presentation interface rendering Resumen, Objetivos de Aprendizaje, Conceptos Clave, Puntos Clave de Repaso, and Glosario de Términos Clave with display-only citation badges (`Pag. X`), coverage disclosure panel ("Páginas con evidencia textual: X de Y"), and Document Library lifecycle badges. Zero raw HTML (React plain text escaping). Verified via Playwright E2E. |
