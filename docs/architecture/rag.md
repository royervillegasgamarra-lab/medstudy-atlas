...
> **Tenant Isolation Invariant**: `document_chunks` does not hold `user_id` directly; ownership is maintained on `documents.user_id`. Tenant isolation is enforced via an explicit `JOIN documents d ON d.id = c.document_id WHERE d.user_id = $3::uuid`.
...
## 5. Provenance & Citation Model

A citation is NOT just an opaque chunk ID. It is a verifiable spatial anchor:
- **`document_id`**: Foreign key to `documents`.
- **`document_title`**: Human-readable title of the syllabus/deck.
- **`page_number`**: Exact page number in the original PDF.
- **`chunk_id`**: Specific text chunk.
- **`quote_snippet`**: Exact 1-2 sentence excerpt confirming the answer.
- **Interactive Action**: Clicking `[Pág. 14]` opens `pdf.js` directly to page 14 and highlights the corresponding bounding box/text.
