...
## 1. Complete Error Taxonomy (22 Error Codes)

### Parser-Reported Error Codes (15)
...
| `PREFLIGHT_FAILED` | No | Preflight diagnostic output exceeded 64 KB per stdout/stderr diagnostic stream or qpdf crashed. |
...
| `OCR_PAGE_LIMIT` | No | Exceeded OCR page limit (`maxOcrPagesPerDocument` = 60 pages). |
| `TEXT_PAGE_LIMIT` | No | Single page extracted/normalized text exceeds `maxExtractedCharsPerPage` (100,000 chars). |
| `TEXT_DOCUMENT_LIMIT` | No | Cumulative extracted text across document exceeds `maxExtractedCharsPerDocument` (3,000,000 chars). |
| `PARSER_INTERNAL_ERROR` | No | Unhandled Python runtime exception or invalid configuration during parsing. |
| `OCR_TIMEOUT` | **Yes** | Single-page OCR exceeded timeout (20s). |
| `OCR_UNAVAILABLE` | No | Tesseract executable not found or required language data (`spa`, `eng`) unavailable. |
| `OCR_FAILED` | No | Tesseract invocation executed but returned/failed with non-timeout execution error. |
...
| `PARSER_OUTPUT_INVALID` | No | Missing manifest, manifest > 64 KB, invalid schema, illegal error code, or semantic provenance check failure. |
...
| **PROC-12** | OCR budget exhaustion (> 60 OCR pages) | Processing halts when OCR page count reaches 61; marked `OCR_PAGE_LIMIT` | Unit (Vitest) | `tests/unit/parser.test.ts` (`fails with OCR_PAGE_LIMIT when OCR budget maxOcrPagesPerDocument is exceeded`) | **PASS** |
...
| **PROC-22** | Single-page character limit (> 100,000 characters) | Page exceeding limit rejected with `TEXT_PAGE_LIMIT`; prevents memory exhaustion | Unit (Vitest) | `tests/unit/parser.test.ts` (`fails with TEXT_PAGE_LIMIT when page extracted text exceeds maxExtractedCharsPerPage`) | **PASS** |
| **PROC-23** | Document character limit (> 3,000,000 characters) | Cumulative document characters exceeding limit rejected with `TEXT_DOCUMENT_LIMIT` | Unit (Vitest) | `tests/unit/parser.test.ts` (`fails with TEXT_DOCUMENT_LIMIT when total extracted text exceeds maxExtractedCharsPerDocument`) | **PASS** |
...
| **PROC-25B** | OCR engine or language unavailable | Tesseract missing or language data absent rejected with `OCR_UNAVAILABLE` | Unit (Vitest) | `tests/unit/parser.test.ts` (`classifies TesseractNotFoundError or missing language as OCR_UNAVAILABLE during DocumentParser.process()`) | **PASS** |
| **PROC-25C** | OCR non-timeout execution failure | Tesseract non-timeout execution error rejected with `OCR_FAILED` | Unit (Vitest) | `tests/unit/parser.test.ts` (`classifies non-timeout Tesseract execution error as OCR_FAILED during DocumentParser.process()`) | **PASS** |
...
| **PROC-32** | Database lease duration default & bounds | Default lease set to 900s; values `< 1`, `> 3600`, or `NULL` raise exception 22023 | Database (pgTAP) | `supabase/tests/database/04_processing_runs_rls.sql` (`Validation: claim_next_processing_run rejects lease_seconds < 1 and > 3600` & `Claim: Lease seconds NULL rejected`) | **PASS** |
...
| **PROC-34** | Bounded qpdf diagnostic output (> 64 KB per stream) | Preflight diagnostic output capped at 64 KB per stdout/stderr diagnostic stream; kills child with SIGKILL and raises ValueError -> `PREFLIGHT_FAILED` | Unit (Vitest) | `tests/unit/parser.test.ts` (`[PROC-34] run_bounded_cmd raises ValueError on stream > 64KB and preflight emits PREFLIGHT_FAILED`) | **PASS** |
...
- **Database Test Suite (`supabase/tests/database/`)**: 245 pgTAP tests passing across 4 suites (55 in `04_processing_runs_rls.sql`).
- **Unit Test Suite (`tests/unit/`)**: 142 unit tests passing across 11 suites (38 in `parser.test.ts`, 8 in `provenance.test.ts`).
...
- **Vitest Total (`pnpm test`)**: 186 tests passing across 13 test files.
