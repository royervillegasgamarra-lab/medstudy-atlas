...
$reviewCriteria = switch -Regex ($PhaseSlug) {
    ...
    "01?c" {
        @(
            ...
            '5. [ ] **Centralized & Concurrency-Safe Quotas**: 25MB max file size, 10 active documents, 100MB total active storage (counting `READY` and unexpired `UPLOADING` reservations within 2-hour lease). Concurrency serialized per user via `pg_advisory_xact_lock`. Declared size must equal actual size.',
            ...
            '9. [ ] **In-Database Security Matrix (pgTAP)**: 52 pgTAP tests in `03_documents_rls.sql` verify table schema, absence of `finalize_token`, anon denials, direct mutation denials, input constraints, quota limits, lease expiration, authenticated archive denial, two-user isolation, and archive idempotency.',
            '10. [ ] **Authoritative Storage API Integration Tests**: 12 authoritative integration tests in `tests/integration/storage-security.test.ts` verify direct upload denial, signed upload success, fake PDF rejection with physical blob cleanup, size mismatch rejection with real object, authenticated archive denial, storage error handling, cross-user download denial, token reuse experiment & lease mitigation, abandoned upload recovery, archive idempotency, and concurrent quota serialization.',
