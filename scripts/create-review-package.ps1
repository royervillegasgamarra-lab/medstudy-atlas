...
$reviewCriteria = switch -Regex ($PhaseSlug) {
    ...
    "01?e" {
        @(
            '1. [ ] **Page-Bounded Deterministic Chunking**: `document_chunks` partitions physical pages without crossing page boundaries (`page_start === page_end`). Target 1800 chars, max 2800 chars, 200-char overlap. Primary keys refetched to guarantee valid database UUID provenance for citations.',
            '2. [ ] **Two-Call LLM Generation & Evidence Verification Pipeline**: Call 1 produces candidate sections (Summary, Objectives, Concepts, High-Yield Points, Key Terms Glossary) with candidate chunk IDs. Call 2 independently verifies textual support against cited chunks (`evidence-verifier.ts`).',
            '3. [ ] **Deterministic Citation Validation (`citation-validator.ts`)**: Model is NEVER trusted to output page numbers. Server validates chunk IDs and derives authoritative page numbers from database chunks. Invalid or unsupplied chunk IDs cause candidate rejection; no quote fuzzy matching in Phase 1E.',
            '4. [ ] **Strict QA Quality Gate**: Rejects candidate packs unless satisfying >= 1 Summary, >= 1 Objective, >= 1 Concept, and >= 50% supported claims (`STUDY_PACK_EVIDENCE_QA_FAILED`).',
            ...
            '6. [ ] **Thin `AIProvider` Abstraction & Mock Provider**: Application decoupled from heavy agentic frameworks. `MockAIProvider` enables deterministic local verification at $0.00 cost; strictly prohibited in production. `OpenAICompatibleProvider` enables production deployment with fail-closed endpoint validation.',
            ...
            '8. [ ] **Database Schema, Composite Foreign Keys & RLS**: 5 tables (`document_chunks`, `study_packs`, `study_pack_items`, `study_pack_item_citations`, `ai_usages`) with composite FKs. Direct mutations revoked; privileged RPCs control queue and writes. 64 pgTAP assertions in `05_chunks_and_study_packs_rls.sql`.',
