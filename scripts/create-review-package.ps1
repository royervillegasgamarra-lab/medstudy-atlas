...
$reviewCriteria = switch -Regex ($PhaseSlug) {
    ...
    "01?d" {
        @(
            ...
            '4. [ ] **Selective Local OCR (`tesseract` 5.5.3)**: Pages with < 50 native characters undergo local Tesseract OCR using strictly `spa+eng` language packs. Render scale capped with pixel limit validation (12M max pixels/page). Max 60 OCR pages enforced per document (`PARSER_RESOURCE_LIMIT`).',
            ...
            '11. [ ] **Automated Test Suites**: 54 pgTAP tests (`04_processing_runs_rls.sql`, 244 total DB tests), 36 unit tests (`parser.test.ts`), 8 provenance tests (`provenance.test.ts`), 17 integration tests (`processing-worker.test.ts`, 184 total Vitest tests), and Playwright E2E tests pass cleanly.',
            '12. [ ] **Zero Cloud Resources & Paid Services**: Local-First execution; $0.00 cost.'
        )
    }
    default {
