...
$reviewCriteria = switch -Regex ($PhaseSlug) {
    ...
    "01?e" {
        @(
            '1. [ ] **Page-Bounded Deterministic Chunking**: `document_chunks` partitions physical pages without crossing page boundaries (single `document_page_id`, `page_number`, `start_char`, `end_char`). Target 1800 chars, max 2800 chars, 200-char overlap. Primary keys refetched to guarantee valid database UUID provenance for citations.',
